...
From Step~4, the magnitudes $|\Gamma_{ij}|$ are known but the signs $\sigma_{ij} = \sgn(\Gamma_{ij}) \in \{+1,-1\}$ remain undetermined. For $n{=}3$ off-diagonal pairs, $\bm{\sigma} = (\sigma_{12}, \sigma_{13}, \sigma_{23})$ yields $2^3 = 8$ candidates.

For each candidate $\bm{\sigma}$, construct the matrix:
\begin{equation}
\bm{D}'_{\bm{\sigma}} = \begin{pmatrix}
W_1 & \sigma_{12}|\Gamma_{12}| & \sigma_{13}|\Gamma_{13}| \\
\sigma_{12}|\Gamma_{12}| & W_2 & \sigma_{23}|\Gamma_{23}| \\
\sigma_{13}|\Gamma_{13}| & \sigma_{23}|\Gamma_{23}| & W_3
\end{pmatrix}
\label{eq:Dsigma}
\end{equation}
Since $\bm{D}'$ is the denominator matrix of the FRF, its eigenvalues are the squared resonance frequencies $\{\omega_{R,1}^2, \omega_{R,2}^2, \omega_{R,3}^2\}$ identified in Step~2. The eigenvalue constraint requires:
\begin{equation}
\mathrm{eig}(\bm{D}'_{\bm{\sigma}}) = \{\omega_{R,r}^2\}_{r=1}^{n}, \quad
\text{i.e.,}\;\; \sum_{r}\bigl|\lambda_r(\bm{D}'_{\bm{\sigma}}) - \omega_{R,r}^2\bigr| < \epsilon
\label{eq:eig_constraint}
\end{equation}
This is verified by checking the characteristic polynomial coefficients: $\tr(\bm{D}'_{\bm{\sigma}})$, $\frac{1}{2}[(\tr \bm{D}'_{\bm{\sigma}})^2 - \tr(\bm{D}'^2_{\bm{\sigma}})]$, and $\det(\bm{D}'_{\bm{\sigma}})$ against the elementary symmetric polynomials of $\{\omega_{R,r}^2\}$.
...
\end{proposition}
\begin{proof}
The similarity transformation $\bm{D}' \to \bm{S}\bm{D}'\bm{S}$ with $\bm{S} = \diag(s_1,s_2,s_3)$, $s_i \in \{+1,-1\}$, preserves eigenvalues but maps $\Gamma_{ij} \to s_i s_j \Gamma_{ij}$. Since $\bm{S}^2 = \bm{I}$, these form a group under multiplication. The kernel $\{\bm{S}: s_i s_j = 1\;\forall\,i<j\} = \{\bm{I}, -\bm{I}\}$ has 2 elements, so the orbit of any sign combination under this action has $|\{+1,-1\}^3|/2 = 4$ elements. The 8 candidates split into exactly 2 orbits of size 4; within each orbit, all members share the same eigenvalues. Only one orbit matches the measured resonances.
\end{proof}

\noindent\textbf{Generalization.} For $n$ DOF, the sign ambiguity involves $\binom{n}{2}$ signs ($2^{n(n-1)/2}$ total). The similarity group $\bm{S} = \diag(s_1,\ldots,s_n)$, $s_i \in \{+1,-1\}$, has $2^n$ elements with kernel $\{\bm{I},-\bm{I}\}$, yielding orbits of size $2^{n-1}$. Hence $2^{n(n-1)/2}/2^{n-1}$ orbits survive, but only $2^{n-1}$ candidates within the correct orbit need evaluation.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{../figures/fig2_offdiag_resolution.pdf}
...
All 4 eigenvalue-consistent candidates share identical $k_i$, $W_i$, and hence identical \emph{diagonal} FRFs $G_{ii}$. However, they produce \emph{different} mass matrices $\bm{M}_{l,\bm{\sigma}}$ and thus different off-diagonal FRFs $G_{ij,\bm{\sigma}}$. The sign of $\Gamma_{ij}$ affects the \emph{entire shape} of $G_{ij}(j\omega)$---the cofactor $C_{ij}(s^2)$ in \eqref{eq:Gij} depends on the signed off-diagonal elements of $\bm{D}'$, not merely isolated anti-resonance dips.

\subsection{Reconstruction: $\bm{D}'_{\bm{\sigma}} \to \bm{M}_{l,\bm{\sigma}}$}

For each surviving candidate $\bm{\sigma}$, reconstruct the signed inverse inertia matrix:
\begin{align}
(\bm{M}_l^{-1})_{ii,\bm{\sigma}} &= \frac{W_i - \omega_{m,i}^2}{k_i}
\label{eq:Mlinv_diag}\\
(\bm{M}_l^{-1})_{ij,\bm{\sigma}} &= \frac{\sigma_{ij}\,|\Gamma_{ij}|}{\sqrt{k_i k_j}}, \quad i \neq j
\label{eq:Mlinv_offdiag}
\end{align}
Then invert to obtain $\bm{M}_{l,\bm{\sigma}} = \bigl[(\bm{M}_l^{-1})_{\bm{\sigma}}\bigr]^{-1}$.

\subsection{Model FRF Computation}

The full coupled FJR system is described by the $2n \times 2n$ impedance matrix. With $s = j\omega$:
\begin{equation}
\bm{Z}_{CL}(s) = \begin{pmatrix}
\bm{J}_m s^2 + \bm{K}_N & -\bm{K}_N \\
-\bm{K}_N & \bm{M}_{l,\bm{\sigma}} s^2 + \bm{K}_N
\end{pmatrix}
\label{eq:Zcl}
\end{equation}
where $\bm{J}_m = \diag(J_{m,i})$, $\bm{K}_N = \diag(k_i/N_i^2)$ (motor-side reflected stiffness), and $\bm{M}_{l,\bm{\sigma}}$ is the candidate link mass matrix. The motor-side $n \times n$ transfer function matrix is extracted from the upper-left block of the inverse:
\begin{equation}
\bm{G}_{\bm{\sigma}}(j\omega) = j\omega \cdot \bigl[\bm{Z}_{CL}^{-1}(j\omega)\bigr]_{1:n,\,1:n}
\label{eq:model_frf}
\end{equation}
where the factor $j\omega$ converts displacement to velocity. This is evaluated at each frequency grid point $\omega_k$.

\subsection{Cost Function and Selection}

The normalized root-mean-square error over all off-diagonal channels:
\begin{equation}
J_{\bm{\sigma}} = \frac{\displaystyle\sqrt{\sum_{k=1}^{N_f}\sum_{i \neq j} \bigl|G_{\bm{\sigma},ij}(j\omega_k) - G_{ij}^{\mathrm{meas}}(j\omega_k)\bigr|^2}}{\displaystyle\sqrt{\sum_{k=1}^{N_f}\sum_{i \neq j} \bigl|G_{ij}^{\mathrm{meas}}(j\omega_k)\bigr|^2}} \times 100\%
\label{eq:nrmse}
\end{equation}
The correct candidate is selected as:
\begin{equation}
\bm{\sigma}^* = \arg\min_{\bm{\sigma}} \; J_{\bm{\sigma}}
\label{eq:sigma_star}
\end{equation}

\subsection{Algorithm Summary}

Algorithm~\ref{alg:main} consolidates Steps~5 and 6 into a single procedure. Step~5 reduces $2^{\binom{n}{2}}$ sign combinations to $2^{n-1}$ candidates via eigenvalue consistency (\S\ref{sec:sign_ambiguity}). Step~6 reconstructs, for each survivor, the full mass matrix $\bm{M}_{l,\bm{\sigma}}$, computes the model FRF via the $2n \times 2n$ impedance inversion, and selects the candidate with minimum off-diagonal NRMSE.
...
\textbf{Step 5:} Eigenvalue filter\;
\ForEach{$\bm{\sigma} \in \{+1,-1\}^{\binom{n}{2}}$}{
    Construct $\bm{D}'_{\bm{\sigma}}$ via \eqref{eq:Dsigma}\;
    \If{$\bigl|\mathrm{eig}(\bm{D}'_{\bm{\sigma}}) - \{\omega_{R,r}^2\}\bigr| > \epsilon$}{
        Discard $\bm{\sigma}$\;
    }
}

\textbf{Step 6:} Off-diagonal FRF fitting\;
\ForEach{surviving $\bm{\sigma}$}{
    $(\bm{M}_l^{-1})_{\bm{\sigma}} \gets$ reconstruct via \eqref{eq:Mlinv_diag}--\eqref{eq:Mlinv_offdiag}\;
    $\bm{M}_{l,\bm{\sigma}} \gets \bigl[(\bm{M}_l^{-1})_{\bm{\sigma}}\bigr]^{-1}$\;
    Build $\bm{Z}_{CL}$ via \eqref{eq:Zcl} with $\bm{M}_{l,\bm{\sigma}}$\;
    \For{$k=1,\ldots,N_f$}{
        $\bm{G}_{\bm{\sigma}}(j\omega_k) \gets j\omega_k \cdot [\bm{Z}_{CL}^{-1}(j\omega_k)]_{1:n,1:n}$\;
    }
    $J_{\bm{\sigma}} \gets$ NRMSE via \eqref{eq:nrmse}\;
}
\Return $\bm{\sigma}^* = \arg\min_{\bm{\sigma}} J_{\bm{\sigma}}$; \quad $\bm{M}_l^{-1} = (\bm{M}_l^{-1})_{\bm{\sigma}^*}$\;
\end{algorithm}

\subsection{Cost and Scaling}

After eigenvalue filtering, $2^{n-1}$ candidates remain. For each, the FRF computation requires inverting the $2n \times 2n$ matrix $\bm{Z}_{CL}(j\omega_k)$ at $N_f$ frequency points: $O((2n)^3 N_f)$ per candidate. Total: $O(2^{n-1} (2n)^3 N_f)$. For $n=3$, $N_f=86$ ($\Delta f=0.5$ Hz, $[2,45]$ Hz): 4 inversions of a $6\times 6$ complex matrix at 86 points---under 1 second.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{../figures/fig3_candidates_frf.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{../figures/fig4_nrmse_bar.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{../figures/fig5_error_comparison.pdf}
