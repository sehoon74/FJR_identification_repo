...
\textbf{Scaling.} For $n$ DOF, the eigenvalue filter retains $2^{n-1}$ candidates. For $n{=}4$: 8; $n{=}5$: 16. Each requires one $O(n^3 N_f)$ FRF computation. Practical for $n \leq 7$.

\textbf{Experimental applicability.} The model FRF in Step~6 depends on identified $k_i$, which has small error from frequency quantization. This adds $\sim 0.2\%$ baseline NRMSE to the correct candidate but does not affect discrimination ($99\%$ gap to incorrect candidates).

%====================================================================
\section{Conclusion}
%====================================================================

We identified joint stiffness and the signed inverse inertia matrix of a multi-DOF FJR without measuring off-diagonal anti-resonances. The method combines Vieta-based magnitude estimation from diagonal FRFs, eigenvalue-based sign filtering ($2^3 \to 2^2$ candidates), and off-diagonal FRF fitting for unique resolution. Results: $k_i$ error $<0.01\%$; all $\bm{M}_l^{-1}$ elements $<0.1\%$ error (signed); sign discrimination $>500\times$ in NRMSE; robust to SNR$=20$\,dB. No nonlinear optimization or iterative procedures are needed.
